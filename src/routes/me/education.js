			...
			<View className='education view'>
				<Card>
					<View className="header">
						<Image className="school" src="https://canisminor.cc/img/resume-education-caa.png" mode="widthFix"
						       lazyLoad/>
					</View>
					{loading ? <Loading/> : education.showcase.map((item, i) => (
						<Cell key={i} title={item.title.toUpperCase()} desc={item.desc} noLine={i === about.showcase.length - 1}/>
					))}
				</Card>
				<WhiteSpace/>
				<Card title="Awards" padding>
					{loading ? <Loading/> : education.awardList.map((items, a) => (
								...
								items.map((item, b) => {
									...
									return (
										<View key={b} className="award">
											...
											<View className="item">
												{award.map((i, c) => <View key={c}>{i}</View>)}
											</View>
										</View>
									);
								})
