...
export default class WXAppPlugin {
	...
	async getEntryResource() {
		const appJSONFile = require(resolve(this.base, 'app.jsx')).default;
		const { pages = [], tabBar = {} } = appJSONFile;
	...
	async getComponents(components, instance) {
		let usingComponents = {}
		try {
			const JSX = require(`${instance}.jsx`).default
			usingComponents = JSX.usingComponents ? JSX.usingComponents : {}
		} catch (err) {
			err && err.code !== 'ENOENT' && console.error(err)
		}
